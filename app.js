...
const BASE_URL = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? '' : '/portfolio';
    ...
    loadAvatar() {
        ...
        loader.load(
            ...
            error => {
                ...
                if (loadingEl) loadingEl.textContent = '3D Avatar unavailable';
            }
        );
    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function startListening() {
        ...
        if (!Recognition) {
            showVoiceStatus('Voice not supported', 'error');
            return;
        }
        ...
        recognition.onresult = event => {
            ...
            showVoiceStatus('Processing...', 'speaking');
            sendToBackend(transcript);
        };

        recognition.onerror = e => {
            console.error(e);
            showVoiceStatus('Voice error', 'error');
        };
        recognition.start();
    }
    ...
    async function sendToBackend(text) {
        try {
            ...
            utter.onstart = () => avatar && avatar.lipSync(1);
            utter.onend = () => avatar && avatar.lipSync(0);
            speechSynthesis.speak(utter);

            if (avatar) avatar.playGesture(emotion);
        } catch (err) {
            console.error('Chat error:', err);
            showVoiceStatus('Connection failed', 'error');
            const fallbackMsg = new SpeechSynthesisUtterance('Sorry, I could not connect to the AI service. Please try again later.');
            speechSynthesis.speak(fallbackMsg);
        }
    }
